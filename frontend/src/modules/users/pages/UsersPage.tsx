...
function UsersPage() {
  ...
  async function fetchUsers() {
    ...
    catch { setError('Erro ao carregar usuários. Verifique se o servidor está rodando.'); }
  ...
  async function handleCreate(payload: CreateUserPayload | UpdateUserPayload) {
    ...
    setModal({ type: 'none' }); await fetchUsers();
  }
  ...
  async function handleDelete() {
    ...
    try { await userService.deleteUser(modal.user.id); setModal({ type: 'none' }); await fetchUsers(); }
  ...
  return (
    <div>
      <div style={styles.header}>
        ...
        <button style={styles.addBtn} onClick={() => setModal({ type: 'create' })}>+ Novo Usuário</button>
      </div>
      ...
      {!loading && users.length === 0 && !error && (
        <div style={styles.emptyState}>
          ...
          <button style={styles.addBtn} onClick={() => setModal({ type: 'create' })}>Cadastrar primeiro usuário</button>
        </div>
      )}
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    <span style={{ ...styles.badge, ...(user.active ? styles.badgeActive : styles.badgeInactive) }}>
                      {user.active ? 'Ativo' : 'Inativo'}
                    </span>
                  </td>
                  <td style={styles.td}>{new Date(user.createdAt).toLocaleDateString('pt-BR')}</td>
                  <td style={{ ...styles.td, textAlign: 'center' }}>
                    <button style={styles.editBtn} onClick={() => setModal({ type: 'edit', user })}>Editar</button>
                    <button style={styles.deleteBtn} onClick={() => setModal({ type: 'delete', user })}>Excluir</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {modal.type === 'edit' && (
        <Modal title="Editar Usuário" onClose={() => setModal({ type: 'none' })}>
          <UserForm user={modal.user} onSubmit={handleEdit} onCancel={() => setModal({ type: 'none' })} />
        </Modal>
      )}
      {modal.type === 'delete' && (
        <ConfirmDialog title="Excluir Usuário"
...
const styles: Record<string, React.CSSProperties> = {
  ...
  badgeAdmin: { backgroundColor: '#eff6ff', color: '#1d4ed8' },
  badgeUser: { backgroundColor: '#f3f4f6', color: '#374151' },
