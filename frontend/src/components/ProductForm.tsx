...
const CATEGORIES = [
  'Eletrônicos', 'Roupas', 'Calçados', 'Acessórios',
  'Casa e Jardim', 'Esportes', 'Brinquedos', 'Livros', 'Outros',
];
...
function ProductForm({ product, onSubmit, onCancel }: ProductFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name) { setError('O nome do produto é obrigatório.'); return; }
    if (!category) { setError('A categoria é obrigatória.'); return; }

    const parsedPrice = parseFloat(price);
    if (!price || isNaN(parsedPrice) || parsedPrice < 0) {
      setError('Informe um preço válido (maior ou igual a zero).'); return;
    }

    const parsedStock = parseInt(stock);
    if (isNaN(parsedStock) || parsedStock < 0) {
      setError('O estoque não pode ser negativo.'); return;
    }

    setLoading(true);
    try {
      await onSubmit({ name, description: description || undefined, price: parsedPrice, stock: parsedStock, category, active });
    } catch (err: unknown) {
      const msg =
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={styles.field}>
        ...
        <input style={styles.input} type="text" value={name}
          onChange={(e) => setName(e.target.value)} placeholder="Ex: Tênis Nike Air Max" disabled={loading} />
      </div>

      <div style={styles.field}>
        <label style={styles.label}>Descrição <span style={styles.optional}>(opcional)</span></label>
        <textarea style={{ ...styles.input, resize: 'vertical', minHeight: '70px' }}
      ...
      <div style={styles.row}>
        <div style={{ ...styles.field, flex: 1 }}>
          ...
          <input style={styles.input} type="number" min="0" step="0.01" value={price}
            onChange={(e) => setPrice(e.target.value)} placeholder="0.00" disabled={loading} />
        </div>
        <div style={{ ...styles.field, flex: 1 }}>
          <label style={styles.label}>Estoque</label>
          <input style={styles.input} type="number" min="0" step="1" value={stock}
            onChange={(e) => setStock(e.target.value)} placeholder="0" disabled={loading} />
        </div>
      </div>
